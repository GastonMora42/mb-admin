...
interface LiquidacionPDFProps {
  liquidacionData: {
    ...
    regularCount: number;
    sueltasCount: number;
    clasesCount: number; // Añadido campo clasesCount
    totalRegular: number;
    totalSueltas: number;
...
const LiquidacionPDF: React.FC<LiquidacionPDFProps> = ({ 
  ...
  <Text style={styles.value}>
    {porcentajesPersonalizados?.porcentajeCursos || 
      ((profesor?.porcentajePorDefecto || 0.6) * 100)}%
  </Text>
</View>
        ...
        <View style={{...styles.infoRow, marginTop: 10}}>
          ...
          <Text style={styles.value}>
            {liquidacionData.sueltasCount} alumnos ({liquidacionData.clasesCount || 0} clases)
          </Text>
        </View>
        <View style={styles.infoRow}>
  <Text style={styles.label}>Porcentaje:</Text>
  <Text style={styles.value}>
    {porcentajesPersonalizados?.porcentajeClasesSueltas || 
      ((profesor?.porcentajeClasesSueltasPorDefecto || 0.8) * 100)}%
  </Text>
</View>
        <View style={styles.infoRow}>
          <Text style={styles.label}>A liquidar:</Text>
      ...
      <View style={styles.table}>
        ...
        {liquidacionData.recibos.map((recibo, index) => (
          <View key={index} style={styles.tableRow}>
            ...
            <Text style={{...styles.tableCol, ...styles.colConcepto}}>
