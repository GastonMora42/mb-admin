// pages/api/descuentos-aplicados.ts
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    ...
    try {
      const { descuentoId, alumnoId, fechaInicio, fechaFin } = req.body;

      // Verificar si ya existe un descuento activo para el alumno
      const descuentoExistente = await prisma.descuentoAplicado.findFirst({
        where: {
          alumnoId: parseInt(alumnoId),
          activo: true,
          descuento: {
            esAutomatico: false // Solo verificamos descuentos manuales
          }
        }
      });
      ...
      const descuentoAplicado = await prisma.descuentoAplicado.create({
        data: {
          descuento: { connect: { id: parseInt(descuentoId) } },
    ...
    try {
      const { id, activo, fechaFin } = req.body;
